...
public class HibernateCustomerRepositoryImpl implements CustomerRepository {
    ...
    public HibernateCustomerRepositoryImpl(EntityManager entityManager) {
    ...
    public List<Customer> find(CustomerQuery query) {
        ...
        return entityManager.createQuery(cq)
                ...
                .map(entry -> new Customer(entry.getKey(),
                        entry.getValue()
                                .stream()
                                .flatMap(customer -> customer.profilePhotoList().stream()).toList()))
                .toList();
    }

    private Predicate[] conditions(CustomerQuery query, CriteriaBuilder cb, Root<CustomerProfilePhotos> root) {
        return Stream.of(
                        query.getIds().map(id -> cb.in(root.get("compositeKey").get("customerId")).value(id)))
